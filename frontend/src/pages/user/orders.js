...
import axios from "axios";
import { io } from "socket.io-client";
import { Package, Truck, CheckCircle, Clock, XCircle, X, Mail, User, Calendar } from "lucide-react";
...
export default function UserOrders() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/orders/user/${encodeURIComponent(email.trim())}`
      );
      if (res.data.success) {
        setOrders(res.data.orders);
        setSearched(true);
          ...
          {ORDER_STATUSES.map((status, index) => {
            ...
            return (
              <div key={status} className="flex flex-col items-center relative">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                    isCompleted ? "bg-green-400 text-white" : "bg-gray-200 text-gray-400"
                  }`}
                >
                  {getStatusIcon(status)}
                </div>
                <span className={`text-xs mt-1.5 text-center max-w-16 ${isCompleted ? "text-gray-700" : "text-gray-400"}`}>
                  {status}
                </span>
              </div>
            );
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-1">Track Your Orders</h1>
          <p className="text-gray-500">Enter your email to view order status</p>
        </div>

        <form onSubmit={handleSubmit} className="mb-8 flex flex-col sm:flex-row justify-center items-center gap-3">
          <input
            type="email"
            placeholder="Enter your email (e.g. john@example.com)"
            value={email}
                  ...
                  <div className="space-y-3">
                    {selectedOrder.products.map((p, i) => (
                      <div key={i} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                        ...
                        <p className="font-semibold">₹{(p.price * p.quantity).toLocaleString()}</p>
                      </div>
                    ))}
                  </div>
